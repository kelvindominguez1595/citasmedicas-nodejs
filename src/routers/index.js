export { default as categoriesRouter } from './categoriesRouter.js';
export { default as gendersRouter } from './gendersRouter.js';
export { default as paymentsRouter } from './paymentsRouter.js';
export { default as reservationsRouter } from './reservationsRouter.js';
export { default as rolesRouter } from './rolesRouter.js';
export { default as statusRouter } from './statusRouter.js';
export { default as userRouter } from './userRouter.js';